...
\[
u_x=\frac{dx}{length},\qquad
u_y=\frac{dy}{length}
\]

If the circle radii are \(r_1\) and \(r_2\):
...
Search, insertion, and deletion depend on the tree height \(h\):
...
- How can a BST be kept balanced to maintain \(O(\log n)\) operations?
...
- BST operations are \(O(h)\), where `h` is the tree height.
